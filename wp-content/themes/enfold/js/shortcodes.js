...
// -------------------------------------------------------------------------------------------
    ...
    function activate_shortcode_scripts(container)
	{
		...
		if($.fn.aviaVideoApi)
		{
			$('.avia-slideshow iframe[src*="youtube.com"], .avia-slideshow iframe[src*="vimeo.com"], .avia-slideshow video').aviaVideoApi({}, 'li');
		}
...
(function($)
{
	...
	$.AviaVideoAPI.defaults  = {
		...
		mute: false,
		controls: false,
		events: 'play pause mute unmute loop toggle reset'
  	...
  	$.AviaVideoAPI.prototype =
    {
		...
		{
			var vid_src = this.$video.get(0).src;
			...
			switch(this.type)
			{
				...
				case "youtube": 
					...
					_self.player = new YT.Player(_self.$video.attr('id'), {
						...
						'onError': function(player){ console.log('YOUTUBE ERROR: '+player); },
		...
		{
			if(this.eventsBound == true || typeof this.eventsBound == 'undefined') return;
...
$.fn.avia_video_section = function()
{
	...
	var win			= $(window),
		...
		calc_height = function(section, counter)
		{
			...
			css_block.text(css_block.text() + css);
		};
  	...
  	$.AviaSlider.prototype =
    {
		...
		_setSize: function(instant)
		{
			...
			var self    		= this,
				...
				{
					slide.find('iframe, embed, video, object').height(video_height + '%');
		...
		_attach_video_events: function()
		{
			...
			self.$slides.each(function(i)
			{
				...
				if(currentSlide.data('avia_video_events') != true)
				{
					...
					currentSlide.on('av-video-events-bound', { slide: currentSlide, wrap: mejs , iteration: i }, onReady);
